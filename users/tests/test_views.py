...
class AdddingAndRemovingAuthorsViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.client.force_login(self.user)
        self.user_input = dict(author_id=self.author_1.id, author_username=self.author_1.username)
    ...
    def test_views_use_redirect(self):
        ...
        self.assertRedirects(
            add_author_resp,
            expected_url=reverse("users:profile", kwargs={"username": self.author_1.username}),
        )
        self.assertRedirects(
            remove_author_resp,
            expected_url=reverse("users:profile", kwargs={"username": self.author_1.username}),
    ...
    def test_responses_have_succses_message(self):
        ...
        self.assertContains(add_author_resp, f"Вы подписались на {self.author_1.username}")
        self.assertContains(
    ...
    def test_view_adds_author_to_user_profile(self):
        ...
        self.client.post(
            reverse("users:add_author_to_profile"),
            data=dict(author_id=self.author_2.id, author_username=self.author_2.username),
            follow=True,
            secure=True,
        )
        self.assertQuerysetEqual(self.user.profile.following.all(), [self.author_1, self.author_2])
