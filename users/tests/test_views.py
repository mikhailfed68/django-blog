...
class SignUpTests(TestCase):
    ...
    def setUp(self):
        ...
        self.resp = self.client.post('/users/register/', data=self.user_input, follow=True, secure=True)
...
class AdddingAndRemovingBlogsViewTests(TestCase):
    ...
    def test_views_use_redirect(self):
        add_blog_resp = self.client.post(reverse('users:add_blog_to_profile'), data=self.user_input, follow=True, secure=True)
        remove_blog_resp = self.client.post(reverse('users:delete_blog_from_profile'), data=self.user_input, follow=True, secure=True)

        self.assertRedirects(add_blog_resp, expected_url=reverse('blog:articles_by_blog', kwargs={'pk': self.blog.pk}))
        self.assertRedirects(remove_blog_resp, expected_url=reverse('blog:articles_by_blog', kwargs={'pk': self.blog.pk}))
    ...
    def test_views_deny_anonymous(self):
        ...
        self.assertRedirects(add_blog_resp, expected_url=f"{reverse('login')}?next={reverse('users:add_blog_to_profile')}")
        self.assertRedirects(remove_blog_resp, expected_url=f"{reverse('login')}?next={reverse('users:delete_blog_from_profile')}")
    ...
    def test_responses_have_succses_message(self):
        ...
        self.assertContains(remove_blog_resp, f'Вы отписались от блога {self.blog.name}')

    def test_views_add_blog_to_user_profile(self):
        self.client.post(reverse('users:add_blog_to_profile'), data=self.user_input, follow=True, secure=True)
        self.assertQuerysetEqual(self.user.profile.blogs.all(), [self.blog])

    def test_views_remove_blog_from_user_profile(self):
        self.client.post(reverse('users:add_blog_to_profile'), data=self.user_input, follow=True, secure=True)
        self.client.post(reverse('users:delete_blog_from_profile'), data=self.user_input, follow=True, secure=True)
...
class ProfileDetailViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.articles = [create_test_article(str(name), self.user) for name in range(13)]
        self.articles.reverse()
        self.resp = self.client.get(reverse('users:profile', kwargs={'username': self.user.username}), secure=True)
    ...
    def test_response_have_private_buttons_for_owner(self):
        ...
        resp = self.client.get(reverse('users:profile', kwargs={'username': self.user.username}), secure=True)
        self.assertContains(resp, 'Создать статью')
        self.assertContains(resp, 'Редактировать')

    def test_pagination_is_ten(self):
        self.assertTrue(self.resp.context['is_paginated'] == True)
        self.assertEqual(len(self.resp.context['object_list']), 10)

    def test_pagination_lists_all_articles(self):
        "Get second page and confirm it has exactly remaining 3 items."
        second_page_resp = self.client.get(reverse('users:profile', kwargs={'username': self.user.username}) + '?page=2' , secure=True)

        self.assertTrue(second_page_resp.context['is_paginated'] == True)
        self.assertEqual(len(second_page_resp.context['object_list']), 3)

    def test_response_has_exactly_user_articles(self):
        other_articles = [create_test_article(str(name), self.another_user) for name in range(20, 23)]
